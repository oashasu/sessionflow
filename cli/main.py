...
from core import get_storage, Task, SessionNote, RemoteHostConfig, Requirement, RequirementSessionLink
from providers import RemoteHost, get_factory
from services import SessionService, RequirementService, MatchingService
...
def cmd_status(args):
    ...
    active_sessions = session_service.get_active()
    ...
    for s in active_sessions:
        print(f"  {s['short_id']}... | {s['project_name']}")
    ...
    if args.req_cmd == "add":
        ...
        try:
            req = req_service.create(
                ...
                work_dirs=args.work_dirs.split(",") if args.work_dirs else [],
            )
        ...
        req = req_service.get_detail(args.req_id)
        if not req:
        ...
        if req_service.update(args.req_id, **kwargs):
            req = req_service.get_detail(args.req_id)
        ...
        if req_service.complete(args.req_id):
...
def cmd_unlink(args):
    ...
    if req_service.unlink_session(args.session_id):
...
def cmd_which_req(args):
    ...
    result = matching_service.get_session_requirement(session.meta.session_id)
    if not result.get('linked'):
        print(f"会话 {session.short_id} 未关联到任何需求")
        return

    if result.get('deleted'):
        print(f"会话 {session.short_id} 关联到已删除的需求: {result['requirement_id']}")
        return

    role_icon = {"primary": "主", "secondary": "辅", "reference": "参"}.get(result.get('role'), "?")
    print(f"会话 {session.short_id} 关联到需求: {result['requirement_id']} - {result.get('requirement_title', '')} [{role_icon}]")
    if result.get('notes'):
...
def main():
    ...
    archive_parser.add_argument("--insight", help="反思总结")
    ...
    restore_parser = subparsers.add_parser("restore", help="恢复已归档会话")
    ...
    delete_parser.add_argument("--force", action="store_true", help="确认删除")
